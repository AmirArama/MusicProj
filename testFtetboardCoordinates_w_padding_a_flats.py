...
for key, (x, y) in fretboard_coordinates.items():
    ...
    fontcolor = "black" if fill_color == 'white' else "white"
    
    # Draw the note text at the centered position
    draw.text(text_position, note, fill=fontcolor, font=font)

    # Generate individual PNG for each unique note, including flats
    if note not in generated_notes:
        # Create and save the sharp note image
        ...
        # Draw the circle for the note image
        ...
        # Save the individual note image as "note_<name>.png"
        ...
        # Create and save the flat equivalent if needed
        if "#" in note:
            flat_note = note.replace("#", "b")
            if flat_note not in generated_notes:
                # Create a new image for the flat note
                flat_image = Image.new("RGBA", note_image_size, (255, 255, 255, 0))  # Transparent background
                flat_draw = ImageDraw.Draw(flat_image)
                
                # Draw the circle for the flat note image
                flat_draw.ellipse([(0, 0), (note_image_size[0] - 1, note_image_size[1] - 1)], fill=fill_color, outline=color)
                
                # Draw the flat note text centered in the small circle
                flat_bbox = flat_draw.textbbox((0, 0), flat_note, font=font)
                flat_text_width, flat_text_height = flat_bbox[2] - flat_bbox[0], flat_bbox[3] - flat_bbox[1]
                flat_text_position = ((note_image_size[0] - flat_text_width) / 2, (note_image_size[1] - flat_text_height) / 2 - font_size * 0.3)
                flat_draw.text(flat_text_position, flat_note, fill=fontcolor, font=font)
                
                # Save the individual flat note image
                flat_filename = f"notes/note_{flat_note}.png"
                flat_image.save(flat_filename)
                generated_notes[flat_note] = flat_filename  # Mark flat note as generated
