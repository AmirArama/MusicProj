...
def write_dict_to_csv(filename, data):
    with open(filename, mode='w', newline='') as file:
        ...
        # Write the header
        writer.writerow(['Interval', 'Ratio Text', 'Ratio Value', 'Cents'])
        # Write the data rows
        for interval, details in data.items():
            writer.writerow([interval, details['ratio_text'], f"{details['ratio_value']:.5f}", f"{details['cents']:.2f}"])

# Write Just Intonation intervals to CSV
write_dict_to_csv('just_intervals.csv', just_intervals)

# Write Equal Temperament intervals to CSV
write_dict_to_csv('equal_temperament_intervals.csv', equal_temperament_intervals)
